...
function init_list() {
  ...
  initial_data.forEach((data) => {
    ...
    for (let name of name_list) data.Owes[name] = 0;
    create(data);
    array.push(data);
...
function transfer_update_array(from_index, amount, to_index) {
  ...
  array[from_index].Owes[array[to_index].name] =
    Math.round(array[from_index].Owes[array[to_index].name] * 100) / 100;
  ...
  array[to_index].Owes[array[from_index].name] =
    Math.round(array[to_index].Owes[array[from_index].name] * 100) / 100;
  display_array();
}
/**
