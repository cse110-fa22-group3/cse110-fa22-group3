...
function init_list() {
  ...
  initial_data.forEach((data) => {
    ...
    radioTransferFrom.innerHTML = `<input type="radio" name="roommate" form="transfer-from"><label>${
      getRoommate(data.id).name
    }</label>`;
    radioTransferTo.innerHTML = `<input type="radio" name="roommate" form="transfer-to"><label>${
      getRoommate(data.id).name
    }</label>`;
