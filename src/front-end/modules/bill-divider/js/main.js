  ...
  for (let [key, value] of Object.entries(data.Owes)) {
    if (key != getRoommate(data.id).name && value != 0)
      wrapper.insertAdjacentHTML(
        "beforeend",
        `<span>Owes $${value} to ${key}!</span><br>`
      );
  }
...
function init_list() {
  ...
  initial_data.forEach((data) => {
    name_list.push(getRoommate(data.id).name);
  });
  ...
  initial_data.forEach((data) => {
    ...
    radioTransferFrom.innerHTML = `<input type="radio" name="roommate" form="transfer_from"><label>${
      getRoommate(data.id).name
    }</label>`;
    radioTransferTo.innerHTML = `<input type="radio" name="roommate" form="transfer_to"><label>${
      getRoommate(data.id).name
    }</label>`;
    radioPay.innerHTML = `<input type="radio" name="roommate" form="pay"><label>${
      getRoommate(data.id).name
    }</label>`;
...
function pay() {
  ...
  function submit(e) {
    ...
    for (let x = 0; x < array.length; x++) if (inputs[x].checked) index = x;
...
function pay_update_array(index, cost) {
  ...
  for (let x = 0; x < array.length; x++) {
    ...
    for (let name in array[x].Owes) array[x].Owes[name] = 0;
  }
  ...
  for (let x = 0; x < array.length; x++) {
    if (array[x].paid < average_paid) {
      for (let name in debtors_percentage) {
        array[x].Owes[name] =
          (average_paid - array[x].paid) * debtors_percentage[name];
  ...
  for (let i = 0; i < array.length; i++) {
    for (let j = i + 1; j < array.length; j++) {
      ...
      else transfer_update_array(j, amount_scd - amount_fst, i);
    }
  }
  display_array();
}
...
function transfer() {
  ...
  transfer_btn.onclick = function () {
    ...
    array[from_index].transferred[getRoommate(array[to_index].id).name] +=
      amount; //store transfer info in transferred array
    ...
    record.innerHTML = `
            <input type="checkbox" form="del_history">
            <span>${
              getRoommate(array[from_index].id).name
...
function transfer_update_array(from_index, amount, to_index) {
  ...
  array[to_index].Owes[getRoommate(array[from_index].id).name] +=
    amount - smaller;
  //round to 2 decimal places, floating point error otherwise
  array[from_index].Owes[getRoommate(array[to_index].id).name] =
    Math.round(
      array[from_index].Owes[getRoommate(array[to_index].id).name] * 100
    ) / 100;
  ...
  array[to_index].Owes[getRoommate(array[from_index].id).name] =
    Math.round(
      array[to_index].Owes[getRoommate(array[from_index].id).name] * 100
    ) / 100;
  display_array();
}
/**
